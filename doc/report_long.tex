...
Across all configurations, AUROC sits in the range $0.95$--$0.97$ on
validation and $0.948$ on the held-out test set. The model has nearly solved
the binary adjacency prediction problem. Figure~\ref{fig:roc-curves} shows the
ROC and precision-recall curves for the best checkpoint; the precision-recall
curve drops more steeply than the ROC, reflecting the low $1.9\%$ positive
rate. Figure~\ref{fig:weight-distribution} reveals a graded \emph{same-source}
signal: cross-image pairs concentrate near zero, adjacent pairs peak around
$0.7$--$0.85$, and same-image non-adjacent pairs fill the intermediate range.
This emergent structure is what the downstream spectral clustering step
exploits.
...
Figure~\ref{fig:failure-modes} shows representative misclassified fragments
from the best checkpoint. Cluster IDs are aligned to source images via
Hungarian matching on the confusion matrix for this visualisation; the model
itself has no notion of source identity. The predominant failure pattern is
near-uniform or repetitive-texture patches that produce similar embeddings
regardless of source image, making them hard to assign correctly from
appearance alone.
