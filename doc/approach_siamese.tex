...
\emph{This section contains placeholders. Numbers are filled in once each run
finishes; \texttt{scripts/compare\_runs.py} renders the comparison table from
\texttt{results.jsonl}.}
...
\begin{tabular}{lcc}
\toprule
                            & \textbf{Best ARI}            & \textbf{Adjacency F1} \\
\midrule
Run 1 (single-batch eval, unbalanced)  & $0.787$           & $0.263$               \\
Run 2 (multi-batch eval, balanced)     & \emph{TBD}        & \emph{TBD}            \\
\bottomrule
\end{tabular}
\end{center}
...
\emph{(Placeholder. Discuss: does balanced clustering close the gap between cluster
sizes? Does multi-batch eval shift the early-stopping iteration? Is the F1/ARI gap
narrower or wider with the new clustering?)}
