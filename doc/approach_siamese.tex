...
A non-trivial fraction of fragments are misclassified even by the best
checkpoint. We inspect representative failures to understand whether the
remaining ARI gap is solvable by a stronger model, or whether it reflects an
intrinsic limit of the data.

\input{fig_failure_modes.tex}

The failures break down into three recurring categories:

\begin{itemize}
    \item \textbf{Near-uniform low-information fragments.} Sky patches, walls,
          smooth dark backgrounds, uniformly coloured regions. Multiple source
          images in any given batch contain such regions, and a $16 \times 16$
          patch from one is essentially indistinguishable from a $16 \times 16$
          patch from another. No amount of additional model capacity will
          resolve this --- the information is not there.
    \item \textbf{Repetitive textures.} Foliage, fur, gravel. Visually
          distinctive at the global scale of an image but ambiguous at a
          $16 \times 16$ window where most patches simply look like ``green
          leaf-shaped texture'' or ``brown fur''. These are partially
          resolvable by a more sophisticated model that can leverage subtle
          colour or scale cues.
    \item \textbf{Edge / boundary fragments.} Patches sitting on the boundary
          between two distinct regions of an image (e.g.\ an animal silhouette
          against grass). The mixed content makes them harder to associate
          with the rest of their source image. These should in principle be
          resolvable.
\end{itemize}

The first category is the dominant one in the figure and represents an
intrinsic ceiling on ARI for any method operating on these
$16 \times 16$ augmented patches without auxiliary information.
