...
other settings fixed: adjacency head only ($\lambda_{\text{adj}} = 1$,
$\lambda_{\text{same}} = 0$, so $\beta_{\text{same}}$ is not used), single
...
(plain BCE on the adjacency head, as in Section~9.3), and
$\beta_{\text{same}} = 1/9.6$ (plain BCE on the same-image head --- the
analogous value for its $\sim$10\% positive rate, corresponding to
\texttt{pos\_weight\_same} $= 1.0$). Single seed throughout. At inference,
when the same-image head is active its confidence scores are used as the
