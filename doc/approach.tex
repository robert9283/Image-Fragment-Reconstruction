...
\label{eq:multi}
\end{equation}
The adjacency head uses $\beta_{\text{adj}} = 1/52$ (plain BCE; the class
imbalance problem motivating $\beta$ is specific to adjacency with its
$\sim$1.9\% positive rate). The same-image head uses unweighted BCE directly:
at $\sim$10\% positives the imbalance is mild and no upweighting is needed.
Equal loss weights $\lambda_{\text{adj}} = \lambda_{\text{same}} = 1$ are used.
...
(plain BCE on the adjacency head, as in Section~9.3), unweighted BCE on the
same-image head (see Section~\ref{sec:loss}), single seed. At inference,
